...
that Tower vends on demand. Whether a catalog is Tower-managed is the server's
call: asking it to vend either returns credentials or refuses with
CatalogNotVendable, and callers fall back to local PyIceberg config on refusal.
Credentials are cached until they near expiry. The one wrinkle is the default
catalog, which is provisioned lazily on first use and needs a short retry loop.
"""
...
_not_vendable: set[tuple] = set()


class CatalogNotVendable(RuntimeError):
    """Tower refused to vend credentials: the catalog is not Tower-managed."""
...
def get_tower_catalog_credentials(
    ...
    if (ctx.tower_url, name, environment) in _not_vendable:
        raise CatalogNotVendable(
            f"Tower catalog {name!r} in environment {environment!r} "
            "is not Tower-managed."
        )
...
def _try_vend(
    ...
    if isinstance(result, ErrorModel) and result.status == 404:
        return None
    if isinstance(result, ErrorModel) and result.status == 422:
        # The vend route refuses catalogs Tower does not manage (e.g. S3 Tables).
        _not_vendable.add((ctx.tower_url, name, environment))
        raise CatalogNotVendable(_error_text(result))
    raise RuntimeError(
