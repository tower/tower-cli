...
"#).unwrap();
    ...
    fn test_from_pyproject_missing_file() {
        let result = TowerfileGenerator::from_pyproject(Some("/nonexistent/path/pyproject.toml"), None);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("pyproject.toml not found"));
    }
...
"#).unwrap();
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("No [project] section found"));
    }
