...
/// struct for passing parameters to the method [`update_account_name`]
#[derive(Clone, Debug)]
pub struct UpdateAccountNameParams {
    /// The name of the account to update
    pub name: String,
    pub update_account_name_params: models::UpdateAccountNameParams,
}
...
/// struct for typed successes of method [`update_account_name`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateAccountNameSuccess {
    Status200(models::UpdateAccountNameResponse),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`update_account_name`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateAccountNameError {
    DefaultResponse(models::ErrorModel),
    UnknownValue(serde_json::Value),
}
...
pub async fn update_account_name(
...
) -> Result<ResponseContent<UpdateAccountNameSuccess>, Error<UpdateAccountNameError>> {
    let uri_str = format!(
        "{}/accounts/{name}",
        configuration.base_path,
        name = crate::apis::urlencode(params.name)
    ...
    req_builder = req_builder.json(&params.update_account_name_params);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<UpdateAccountNameSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            tower_trace_id,
            status,
            content,
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateAccountNameError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            tower_trace_id,
            status,
            content,
