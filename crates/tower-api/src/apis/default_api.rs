...
/// struct for passing parameters to the method [`list_plans`]
#[derive(Clone, Debug)]
...
/// struct for typed successes of method [`list_plans`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListPlansSuccess {
    Status200(models::ListAccountPlansResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn list_plans(
...
) -> Result<ResponseContent<ListPlansSuccess>, Error<ListPlansError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        let content = resp.text().await?;
        let entity: Option<ListPlansSuccess> = serde_json::from_str(&content).ok();
        Ok(ResponseContent {
            tower_trace_id,
            status,
            content,
        ...
        let content = resp.text().await?;
        let entity: Option<ListPlansError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            tower_trace_id,
            status,
            content,
